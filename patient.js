...
(function() {
    ...
    window.HolaDocPatient = {
        async renderDashboard(container) {
            ...
            const allRequests = await window.HolaDocStorage.getRequests({ patientDni: user.dni });
            const requests = allRequests.slice(0, 5);
            const doctors = await window.HolaDocStorage.getDoctors();
            const docMap = {};
            doctors.forEach(d => { docMap[d.dni] = d.name; });
                        ...
                        ` : `
                            <div class="card" style="padding: 0; overflow: hidden;">
                                ${requests.map(r => {
                                    ...
                                    const docName = docMap[r.doctorDni] || 'Médico General';
        ...
        async renderAppointment(container) {
            ...
            function renderDoctors(docList) {
                ...
                return docList.map(d => {
                    ...
                    const days = Object.keys(schedule).filter(k => schedule[k] && schedule[k].active).map(k => DAYS_SPANISH[k]).join(', ');
                    return `
                        <div class="card card-3d select-doctor-btn" data-dni="${d.dni}" style="cursor:pointer; text-align:left;">
                            ...
                            <p class="text-muted" style="font-size:15px; margin:4px 0 0 0;"><b>Duración:</b> ${d.consultationDuration || 30} min por consulta.</p>
                        </div>
                    `;
                }).join('');
            }
            ...
            function renderStep2() {
                ...
                document.getElementById('btn-next-step3').addEventListener('click', async () => {
                    ...
                    const worksToday = schedule[dayKey] && schedule[dayKey].active;
            ...
            async function renderStep3(dayKey) {
                const schedule = (selectedDoctor.schedule || {})[dayKey] || { start: '08:00', end: '16:00' };
            ...
            function renderStep4() {
                ...
                document.getElementById('btn-confirm-appointment').addEventListener('click', async () => {
                    ...
                    // Notify doctor
                    await window.HolaDocNotifications.createNotification(
                        selectedDoctor.dni,
                        'doctor',
                        'info',
            ...
            if (type === 'receta') {
                ...
                formHTML = `
                    <div class="card mb-3" style="background:#EBF8FF; border-left: 6px solid var(--primary); text-align:left;">
                        ...
                        <p style="margin:4px 0; font-size:16px;"><b>Obra Social/Prepaga:</b> ${user.obraSocial || 'Sin especificación'}</p>
        ...
        async renderHealth(container) {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                await window.HolaDocStorage.saveHealthData({
                    ...
                    bloodPressureSys: document.getElementById('h-sys').value,
                    bloodPressureDia: document.getElementById('h-dia').value,
                    glucose: document.getElementById('h-glucose').value || null,
                    weight: document.getElementById('h-weight').value || null,
                    temperature: document.getElementById('h-temp').value || null,
                    heartRate: document.getElementById('h-hr').value || null,
                    notes: document.getElementById('h-notes').value.trim()
                });

                window.HolaDocApp.showToast('¡Registro guardado correctamente!', 'success');
                await this.renderHealth(container);
            });
        },
        ...
        async renderHistory(container) {
            ...
            // Merge requests and appointments
            const allItems = [
                ...requests.map(r => ({ ...r, isAppt: false })),
                ...appointments.map(a => ({ ...a, isAppt: true, type: 'turno' }))
            ...
            async function renderTabs(activeTab) {
                    ...
                    if (allItems.length === 0) {
                        tabContent = `
                            <div class="empty-state">
                                ...
                                <h3 class="empty-state-title">Sin solicitudes registradas</h3>
                                <p class="text-muted">Acá vas a ver tus pedidos de recetas, derivaciones y turnos.</p>
                            </div>
                        `;
                    } else {
                        tabContent = `
                            <div class="grid" style="gap:16px;">
                                ${allItems.map(r => {
                                    ...
                                    if (r.isAppt) {
                                        icon = '📅';
                                        typeName = 'Turno Médico';
                                        detailsPreview = `Fecha: ${new Date(r.date).toLocaleDateString('es-AR')} a las ${r.time} hs`;
                                    } else {
                                        if (r.type === 'receta') { 
                                            icon = '💊'; 
                                            typeName = 'Receta Digital'; 
                                            detailsPreview = r.details.medications;
                                        } else if (r.type === 'derivacion') { 
                                            icon = '🏥'; 
                                            typeName = 'Derivación Médica'; 
                                    ...
                                    const docName = docMap[r.doctorDni] || 'Médico General';
                                    const dateStr = new Date(r.createdAt || r.date).toLocaleDateString('es-AR');
                                        ...
                                        <div class="card card-3d" style="text-align:left; border-left:6px solid var(${borderCol});">
                                            ...
                                            ${r.status === 'cancelado' && r.cancelReason ? `
                                                <div style="margin: 8px 0; padding: 10px 14px; background: #FEF2F2; border: 1px solid #FCA5A5; border-radius: 8px; color: #991B1B; font-size:15px;">
                                                    ⚠️ <b>Motivo de la cancelación:</b> ${r.cancelReason}
                                                </div>
                                            ` : ''}
                ...
                container.innerHTML = `
                    <div class="page-enter">
                        ...
                        <!-- Tab controls -->
                        <div class="tabs">
                            <div class="tab ${activeTab === 'requests' ? 'tab--active' : ''}" id="tab-btn-requests">📋 Mis Pedidos (${allItems.length})</div>
        ...
        async renderRequestDetail(container, requestId) {
            ...
            const allRequests = await window.HolaDocStorage.getRequests();
